...
def chart_5() -> None:
    """Chart 5: bar chart, debt service payments over time"""
...
def _calculate_debt_service_pct_gov_spending(ds_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ds_df.merge(
            gov_df.rename(columns={"value": "gov_spending"}),
            how="left",
            on=["entity_code", "year"],
            validate="one_to_one",
        )
...
def chart_6() -> None:
    ...
    ds_df = (
        ds_df.loc[
            lambda d: (d.creditor_name == "All creditors") & (d.year <= LATEST_YEAR)
        ]
    ...
    ds_df = ds_df.merge(
        (
            health_df.loc[:, ["entity_code", "year", "value"]].rename(
                columns={"value": "health_expenditure"}
            )
        ),
        how="left",
        on=["entity_code", "year"],
    ...
    ds_df = ds_df.merge(
        (
            education_df.loc[:, ["entity_code", "year", "value"]].rename(
                columns={"value": "education_expenditure"}
            )
        ),
        how="left",
        on=["entity_code", "year"],
    ...
    # final formatting
    ds_df = (
        ds_df.drop(columns="value")
        .rename(
            columns={
                "ds_pct_gov_spending": "debt service",
...
def key_stat_chart_1() -> None:
    """Key stat chart 1: Africa total debt service"""
...
def key_stat_chart_2() -> None:
    """Key stat chart 2: Africa total debt stocks"""
...
def key_stat_chart_3() -> None:
    """Key stat chart 3: Africa median debt stocks % of GDP"""
...
class KeyStats:
    ...
    def dsa_stats(self) -> None:
        ...
        df = df.assign(
            region=lambda d: places.resolve_places(
                d.country_name, to_type="region", not_found="ignore"
            )
        ).loc[:, ["country_name", "region", "risk_of_debt_distress"]]

        # Africa number in/at high risk of debt distress
        africa_num = df.loc[
            lambda d: (d.region == "Africa")
            & (d.risk_of_debt_distress.isin(["High", "In debt distress"]))
        ...
        africa_share = int(
            round(
                ...
                / len(
                    df.loc[
                        lambda d: d.risk_of_debt_distress.isin(
    ...
    def debt_stocks_stats(self) -> None:
        ...
        self.key_stats["debt_stocks_africa"] = format_values(
            vals.value
        )  # total debt stocks for Africa in the latest year
    ...
    def debt_stock_category_stats(self) -> None:
        ...
        category_vals = (
            ...
            .groupby(["category"])
            .agg({"value": "sum"}, observed=True)["value"]
            .to_dict()
        )
        ...
        self.key_stats["debt_stocks_africa_bilateral_pct"] = (
            f"{int(round(bilateral_val, 0))}%"
        )
        self.key_stats["debt_stocks_africa_multilateral_pct"] = (
            f"{int(round(multilateral_val, 0))}%"
        )
        self.key_stats["debt_stocks_africa_private_pct"] = (
            f"{int(round(private_val, 0))}%"
        )
